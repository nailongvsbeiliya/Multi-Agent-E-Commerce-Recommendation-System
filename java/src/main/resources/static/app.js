...
function setStatus(type) {
    ...
    statusBadge.textContent = statusMap[type];
}
    ...
    grid.innerHTML = products.map(product => `
        <article class="product-card">
            ...
            <div class="product-tags">
                ...
                <span class="pill">评分 ${product.score}</span>
            </div>
        </article>
    `).join("");
}
    ...
    grid.innerHTML = entries.map(([key, value]) => `
        <article class="agent-card ${value.success ? "success" : "failed"}">
            <div class="agent-meta">
                <h3>${key}</h3>
                <span class="pill">${value.latencyMs?.toFixed?.(1) ?? value.latencyMs ?? 0} ms</span>
            </div>
            <p>${value.success ? "执行成功" : `执行失败: ${value.error || "unknown"}`}</p>
            <div class="agent-tags">
                <span class="pill">confidence ${value.confidence}</span>
...
function applyResponse(data) {
    ...
    document.getElementById("experimentStrategy").textContent = data.experimentInfo?.strategy || "无策略";
    document.getElementById("latency").textContent = data.totalLatencyMs ? data.totalLatencyMs.toFixed(1) : "-";
...
form.addEventListener("submit", async (event) => {
    ...
    try {
        ...
        console.error(error);
        setStatus("error");
        document.getElementById("experimentInfo").textContent = error.message;
    }
});
